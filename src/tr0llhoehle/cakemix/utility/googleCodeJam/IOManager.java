...
public class IOManager<S extends Solver<P>, P extends Problem> {
    ...
    private P createProblem() throws InstantiationException, IllegalAccessException {
	...
	if (filePath.length() >= 3 && filePath.substring(filePath.length() - 3).equals(".in")) {
	    filePath = filePath.substring(0, filePath.length() - 3);
	}

	out = new File(filePath + ".out");
	out.createNewFile();
	...
	for (int probID = 0; probID < amountOfProblems; probID++) {
		...
		for (SupportedTypes t : probTypes) {
			...
			break;
		    default:
			System.err.print("Default case! This is not a good sign... ");
		    }
		}
	...
	for (int i = 0; i < problems.size(); i++) {
	    writer.append("Case #" + i + ": " + solver.solve(problems.get(i)) + (char) Character.LINE_SEPARATOR);
	}
	System.out.println("Done!");
